	...
	function generate_body_code($id, $content, $has_wrapper) {
		...
		if ($item_row != null)
		{
			...
			$valOptions = array('heightmode', 'minheight', 'firstid', 'direction', 'tabposition', 'tabiconposition', 'horizontaltabalign', 'hidetitleonsmallscreenwidth', 'transition',
					'responsivemode', 'tabarrowmode', 'horizontalarrowwidthsameasheight', 'horizontalarrowwidth', 'arrowprevicon', 'arrownexticon', 'dropdownmenutext', 'dropdownmenuicon', 'triggerresizeonloadtimeout', 'fullwidthtabsonsmallscreenwidth');
				...
				{		
					...
					else if ($slide->tabicon == 'image')
					{
						...
						if (strlen($slide->tabiconimage) > 0)
						{
							...
							$ret_header .= '<img class="wonderplugintabs-header-icon-image" src="' . $slide->tabiconimage . '" style="width:' . $img_w . 'px;height:' . $img_h . 'px;" />';							
						}
