...
class WonderPlugin_Tabs_List_Table extends WP_List_Table {
	...
	function get_columns()
	{
		$columns = array(
				...
				'id' => __('ID', 'wonderplugin_tabs'),
				'name' => __('Name', 'wonderplugin_tabs'),
				'shortcode' => __('Shortcode', 'wonderplugin_tabs'),
				'phpcode' => __('PHP code', 'wonderplugin_tabs'),
				'time' => __('Created', 'wonderplugin_tabs')
		);
		return $columns;
	}
	...
	function column_default( $item, $column_name ) 
	{
		...
		switch( $column_name ) {
			case 'id':
				...
				else
				{
					$actions = array(
							...
							'view' => sprintf('<a href="?page=%s&itemid=%s">View</a>', 'wonderplugin_tabs_show_item', $item['id']),
							'edit' => sprintf('<a href="?page=%s&itemid=%s">Edit</a>', 'wonderplugin_tabs_edit_item', $item['id'])
					);
				}
			...
			case 'shortcode':
				return esc_attr('[wonderplugin_tabs id="' . $item['id'] . '"]');
			case 'phpcode':
					return esc_attr('<?php echo do_shortcode(\'[wonderplugin_tabs id="' . $item['id'] . '"]\'); ?>');
			default:
				return $item[ $column_name ];
		}
	}
	...
	function get_views(){
		...
		// All
		$all_url = admin_url('admin.php?page=wonderplugin_tabs_show_items');
		$class = ($current == 'all' ? ' class="current"' :'');
		$views['all'] = "<a href='" . $all_url . "' " . $class . " >All (" . $this->published . ")</a>";
	
		// Published
		$publish_url = admin_url('admin.php?page=wonderplugin_tabs_show_items&item_status=publish');
		$class = ($current == 'publish' ? ' class="current"' :'');
		$views['publish'] = "<a href='" . $publish_url . "' " . $class . " >Published (" . $this->published . ")</a>";
	
		// Trash
		$trash_url = admin_url('admin.php?page=wonderplugin_tabs_show_items&item_status=trash');
